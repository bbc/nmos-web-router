...
let ClearConfirm = ({changes, actions, senders}) => {
  ...
  let clearClick = () => {
    ...
    changes.forEach(change => {
      if (change.oldSender) actions.unstageChange(change.sender.id, change.receiver.id, change.type, change.oldSender.id)
      else actions.unstageChange(change.sender.id, change.receiver.id, change.type)
    })
  ...
  let confirmClick = () => {
    changes.forEach((change) => {
      if (change.type === 'route') {
        actions.deployRoute(senders, change.sender.id, change.receiver, change.oldSender.id)
